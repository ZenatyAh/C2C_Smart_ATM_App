...
export default function Deposit() {
  const { user, balance, deposit, loadUser, isLoading } = useAccount();
  ...
  const handleDeposit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      deposit(value); 
      showToast(`✅ Deposited ${value} ILS successfully!`, "success");
      setAmount("");
    } catch (err) {
      ...
      <div className="bg-card border border-border rounded-[var(--radius)] shadow-md p-8 w-full max-w-md">
        ...
        <form onSubmit={handleDeposit} className="space-y-5">
          ...
          <button
            ...
            className="w-full py-2 bg-primary text-primary-foreground font-semibold rounded-[var(--radius)] hover:bg-emerald-600 transition-colors disabled:opacity-50"
          >
